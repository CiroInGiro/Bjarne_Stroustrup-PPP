...
Graph_lib::TBox::TBox(Point xy, const string& titolo) : Box({ 0,0 }, 0, 0), _xy{ xy } {
	set_label(titolo);
...
// Modifiche per WumpusGUI
// aggiungo la possibilità di eliminare le righe
void Graph_lib::TBox::clear_line() { _linee.clear(); }
...
void Graph_lib::TBox::set_color(Graph_lib::Color c) {
	Box::set_color(c);
	_titolo.set_color(c);
}
...
void Graph_lib::TBox::resize() {
	...
	_height = (riga_height + (riga_height * _linee.size()) + (5 * _linee.size()));
