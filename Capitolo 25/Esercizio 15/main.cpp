...
#include "My_Stack.h" // gestore dello stack di oggetti diversi
...
int main(int argc, char** argv) {
	...
	//std::stack<char*> pila;
	My_Stack<(10000 * 1024)> pila; // 10000 allocazioni alla dimensione massima di 1Kb
	...
	for (size_t misure = 1; misure <= 5; ++misure) {
		...
		for (size_t allocazioni = 1; allocazioni <= 10000; ++allocazioni) {
			int dim = rnd_dim(rnd_engine);
			void* test = pila.get(dim); // non ci interessa conservare il puntatore perché non lo utilizziamo
			if (!test) throw; // allocazione fallita
		}
		// prendiamo il tempo finale
		time_stop = std::chrono::system_clock::now();
		time_now = std::chrono::system_clock::to_time_t(time_stop);
		...
		std::cout << "Inizio del test " << (num_test + 1) << ": " << time_str << std::endl;
		// esecuzione del test
		while (pila.available() < pila.size()) pila.free(); // finché non è libero tutto lo spazio ci saranno allocazioni da liberare
		// prendiamo il tempo finale
		time_stop = std::chrono::system_clock::now();
		time_now = std::chrono::system_clock::to_time_t(time_stop);
		localtime_s(&time_struct, &time_now);
