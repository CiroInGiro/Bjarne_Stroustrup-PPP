...
// --------------------------------------------------------------------------------------
// widget per campi di IO delle righe
int Graph_lib::In_Out_Box::get_int() {
	Fl_Input& pi = reference_to<Fl_Input>(pw);
	const char* p = pi.value();
	if (!isdigit(p[0]) && p[0] != '-') return -999999;
	return atoi(p);
}

std::string Graph_lib::In_Out_Box::get_string() {
	Fl_Input& pi = reference_to<Fl_Input>(pw);
	return std::string(pi.value());
}

void Graph_lib::In_Out_Box::put(int i) {
	std::ostringstream oss{ "" };
	oss << i;
	Fl_Input& pi = reference_to<Fl_Input>(pw);
	pi.value(oss.str().c_str());
}

void Graph_lib::In_Out_Box::put(const std::string& s) {
	Fl_Input & pi = reference_to<Fl_Input>(pw);
	pi.value(s.c_str());
}

void Graph_lib::In_Out_Box::attach(Window& win) {
...
righe_ordine::righe_ordine(Point pos, int width, int height, int p1, int p2, int p3) : Widget(pos, width, height, "", 0) {
	...
	_prodotto = new Graph_lib::In_Out_Box(Point{ dx, dy }, wdesc, height, "");

	// posizione la quantita
	dx += (int)(wdesc + wmargin);
	_quantità = new Graph_lib::In_Out_Box(Point{ dx, dy }, wqty, height, "");

	// posiziono il campo per il prezzo unitario
	dx += (int)(wqty + wmargin);
	_prezzo = new Graph_lib::In_Out_Box(Point{ dx, dy }, wprezzo, height, "");
}
...
void righe_ordine::set_riga(const Purchase& p) {
	_prodotto->put(p.prodotto);
	_prezzo->put(p.unit_price);
	_quantità->put(p.count);
}
// --------------------------------------------------------------------------------------
	...
	_salva(Point{ 0, 0 }, 120, 25, "Salva", cb_salva)
{
	...
	// Se _order passato al costruttore già è valido allora lo visualizziamo
	if (*_ordine) {
		_cliente.put(_ordine->cliente);
		_indirizzo.put(_ordine->address);
		// dobbiamo faro lo stream della data o aggiungere la funzione membro che ne restituisce il formato stringa
		_data.put(_ordine->data_ordine.to_string());
	}
	...
	// Se abbiamo un valido Order allora riempiamo le righe da Order passato al costruttore di InserimentoOrdine
	if (*_ordine) {
		unsigned int indice{ 0 };
		unsigned int count_next{ 0 };
		for (Purchase p : _ordine->acquisti) {
			_righe[indice]->set_riga(p);
			++indice;
			if (indice >= _righe.size()) {
				next(); ++count_next;
			}
		}
		// mi riporto in cima alla lista degli articoli
		while (count_next > 0) {
			prev();
			--count_next;
		}
...
void InserimentoOrdini::go() {
	while (gira) {
		Fl::wait(1000);
		// Se chiudo la finestra fermo il loop
		// catturare evento per uscire
		// altrimenti se si lancia Fl::run() gli enenti di un window sono gestiti da fltk
		// ma nella versione di inserimento abbiamo bisogno di controllare noi gli eventi per sapere se il salvataggio
		// è andato a buon fine.
		if (Fl::event() == FL_CLOSE) gira = false;
		// FL_SHORTCUT è quando si preme ESC o uso shortcut, FL_CLOSE avviene alla chiusura della finestra
	}
}
// --------------------------------------------------------------------------------------