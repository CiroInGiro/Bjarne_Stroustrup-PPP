...
Definisci una versione della funzione word-counting dove l'utente può specificare un insieme di caratteri spazio da utilizzare.
*/
...
int count_words_white(Document& d, const std::string& whites) {
    ...
    while (t != d.end()) {
        for (const char c : whites) {
            if (c == *t) {
                // con questo if salto più caratteri bianchi di seguito
                if (parola.size() > 0) {
                    ++retVal;
                    parola = "";
                    whitespace = true;
