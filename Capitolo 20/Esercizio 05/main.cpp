/*
Capitolo 20 Esercizio 5
Definisci un input e un output operator ( >> e << ) per vector.
*/
...
std::vector<T>* get_from_jill() {
	...
	T val = T();
	char delimiter{ ' ' };
	while (ifs >> *retVal >> delimiter) { if (delimiter != '|') return nullptr; else retVal->push_back(val); }
	if (ifs.bad() && !ifs.eof()) return nullptr;
	if (ifs.fail() && !ifs.eof()) return nullptr;
	ifs.close();
	return retVal;
